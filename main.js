...
const floors =  Array.apply(null, Array(numOfFloors)).map((f, i) => {
    //                      width,        height,               top,                         left, id, name
    return (new Floor(elementWidth, elementHeight, (numOfFloors - i - 1) * (elementHeight * 2), 0, i, i))
})

const elevators =  Array.apply(null, Array(numOfElevators)).map((e, i) => {
    //                      width,        height,             top,                        left,                           id,             color, available, speed
...
const buttons =  Array.apply(null, Array(numOfFloors)).map((b, i) => {
    //                      width,      height,                 top,                left,                                                   id,                             color, state, fontColor
